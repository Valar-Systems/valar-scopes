...
import type { DriftStatus } from "./drift";
...
export interface TriageInput {
  ...
  otaNotOk: OtaRow[]; // OTA results other than "ok", 30 days
  drift: DriftStatus;
}
...
export function computeTriage(t: TriageInput): TriageItem[] {
  ...
  const drift = t.drift;
  const driftBad = drift.state !== "CLEAN";
  ...
  return [
    ...
    },
    {
      ...
      devices: [],
      detail: [],
      note: driftBad ? `${drift.state}${drift.detail ? ` -- ${drift.detail}` : ""}` : undefined,
    },
  ];
}
