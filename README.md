...
**We collect anonymous usage counts, and we do not collect what they were about.** Since firmware 9 the device reports, at most once an hour on a request it was already making, a short line of numbers: how many times a detail card was opened, how many times each screen was switched to, how many logbook claims, whether Follow is switched on (a yes/no, never the aircraft), and how long the device has been running. That is the whole payload — eight numbers separated by commas. It tells us which features get used so we know what to improve.

**What it cannot contain.** Not which aircraft you opened, not the callsign or tail number, not the aircraft you follow, not your location, not your configuration, not a timestamp per action. This is enforced by construction rather than by care: the code that builds the report takes only integers, so there is no parameter through which a name could reach it, and the test suite asserts the payload is digits and commas and nothing else. The server rejects anything of another shape.

**This is a change, and we would rather say so than reword it quietly.** Earlier firmware collected none of this, and this section used to say we never would. We changed our minds: at fleet scale we could not tell a feature nobody wanted from a feature nobody could find, and asking ten owners does not scale past ten. What has not changed, and will not, is the line between counting *that* a feature was used and recording *what* it was used on. If you would rather send nothing at all, run the device against your own receiver — then it never talks to us.

What we *can* see is the shape of the traffic a cloud device has to send anyway — that it checked in, from which model, on which firmware. We use that to notice a device that has stopped talking or a firmware update that didn't land. That is a consequence of serving you data, not a measurement of you.

**The opt-out is architectural, not a toggle — with one thing to know if you run your own receiver.** If you run Blipscope against **your own ADS-B receiver** (above) or your own self-hosted proxy, the radar itself never talks to our servers: every aircraft position comes off your hardware, and that isn't a setting we promise to honour, it's a mode where the path doesn't exist.

**Detail cards are the exception, and you choose — there's no default.** Your receiver reports positions, but not what an aircraft *is* — type, airline, route, photo. That has to come from somewhere, so local-receiver setups make you pick an **Aircraft details** setting up front. It ships unselected, and until you choose, details stay off. Nothing here ever changes on its own — not on an update, not ever:

- **Blipscope Cloud** — when you tap an aircraft, we're sent that aircraft's ICAO hex, its callsign and its position, along with your device model, firmware version and access key. **Not** your receiver's address, and **not** your configured location. Be aware, though, that an aircraft you tapped is by definition near you — treat this as coarse location rather than none. Heap/update telemetry is *not* sent on this path; it rides only the cloud-feed connection, which you don't have. In exchange you get cached routes, proper type names and the photo library. This *replaces* the third-party connection below rather than adding to it, so details cost one internet host instead of two.
- **adsbdb direct** — queries the public api.adsbdb.com instead (plus a second host for thumbnails). We're never contacted; a third party is. No photo library.
- **Off** — nothing external is contacted at all. The card shows only what your own receiver reported.

Pick **Off** and the "transmits nothing, by construction" claim holds completely. The firmware is open source, so all three paths are auditable rather than something you have to take on trust.

One consequence worth knowing: a local-receiver device never asks us for its settings either, so fleet-wide tuning we push to cloud devices simply doesn't reach it. Yours behaves the same tomorrow as today unless *you* change it or install a firmware update.

**The spotting leaderboard is separate, opt-in, and off by default.** If — and only if — you turn on the leaderboard on the config page, an opted-in device sends, once an hour, a tiny submission: a salted hash of its MAC (never the raw MAC), your chosen spotter name, and your Logbook **counts** — how many unique aircraft types, airlines, countries, and route airports you've logged — plus the **list of ICAO type codes** you've seen (needed to score rarer types higher). That's everything. It never sends your location, the specific flights or tail numbers you've watched, timestamps of sightings, or the airline/country/airport *lists* (those could hint at where you are — only the type list, which can't, is sent). Leave the toggle off and none of this happens; the reporting path, like the telemetry above, simply isn't exercised.
