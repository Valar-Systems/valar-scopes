...
**Detail cards are the exception, and you choose.** Your receiver reports positions, but not what an aircraft *is* — type, airline, route, photo. That has to come from somewhere, so local-receiver setups have an **Aircraft details** setting:

- **Blipscope Cloud** *(default)* — when you tap an aircraft, we're sent that aircraft's ICAO hex, its callsign and its position, along with your device model, firmware version and access key. **Not** your receiver's address, and **not** your configured location. Be aware, though, that an aircraft you tapped is by definition near you — treat this as coarse location rather than none. In exchange you get cached routes, proper type names and the photo library. This *replaces* the third-party connection below rather than adding to it, so details cost one internet host instead of two.
