...
echo
echo "== NetWatchPolicy =="
# The reachability ladder. Host-tested because the DECISIONS are a pure function
# of two counters and a clock, and the case counts here (both trigger conditions
# independently, every recovery point, a refused reboot, a 20-minute backoff, and
# the WL_CONNECTED control) are not reachable in an afternoon on a bench. The
# ACTIONS are proved on hardware instead -- see the rehearsal in
# docs/ota-control-plan.md.
if ! "$CXX" $FLAGS $INCLUDES       "$ROOT/test/host/test_net_watch_policy.cpp"       -o "$OUT/test_net_watch_policy.exe" 2>"$OUT/build.log"; then
  echo "FAIL: the net-watch-policy test did not compile"
  cat "$OUT/build.log"
  exit 2
fi
"$OUT/test_net_watch_policy.exe"
rc=$?
if [ "$rc" -eq 127 ] || [ "$rc" -gt 2 ]; then
  echo "FAIL: the binary did not run (exit $rc). This is the RIG, not the code."
  exit 2
...
echo
echo "== QuietHourPolicy =="
# The daily reboot schedule. Host-tested because its two hazards -- an hour that
# happens twice and an hour that never happens -- reach this device via a CONFIG
# EDIT (there are no DST rules; see the header), which would take six months to
# observe twice on a bench. And because the offset arithmetic is where a
# truncating division silently lands a day out for the whole western hemisphere.
if ! "$CXX" $FLAGS $INCLUDES       "$ROOT/test/host/test_quiet_hour_policy.cpp"       -o "$OUT/test_quiet_hour_policy.exe" 2>"$OUT/build.log"; then
  echo "FAIL: the quiet-hour-policy test did not compile"
  cat "$OUT/build.log"
  exit 2
fi
"$OUT/test_quiet_hour_policy.exe"
rc=$?
if [ "$rc" -eq 127 ] || [ "$rc" -gt 2 ]; then
  echo "FAIL: the binary did not run (exit $rc). This is the RIG, not the code."
  exit 2
...
# failure at an exact moment, and because the retry's own hazard (two rows for
# one boot) is invisible on a bench that only ever sees the happy path.
if ! "$CXX" $FLAGS $INCLUDES       "$ROOT/test/host/test_boot_report_policy.cpp"       -o "$OUT/test_boot_report_policy.exe" 2>"$OUT/build.log"; then
  echo "FAIL: the boot-report-policy test did not compile"
  cat "$OUT/build.log"
  exit 2
fi
"$OUT/test_boot_report_policy.exe"
rc=$?
if [ "$rc" -eq 127 ] || [ "$rc" -gt 2 ]; then
  echo "FAIL: the binary did not run (exit $rc). This is the RIG, not the code."
  exit 2
...
echo
echo "== LocalOffset =="
# ONE derivation of local time, because there were two and the second dropped
# the longitude fallback -- scheduling every unset board's 03:00 reboot at 03:00
# UTC. The first case in the file is that branch.
if ! "$CXX" $FLAGS $INCLUDES       "$ROOT/test/host/test_local_offset.cpp"       -o "$OUT/test_local_offset.exe" 2>"$OUT/build.log"; then
  echo "FAIL: the local-offset test did not compile"
  cat "$OUT/build.log"
  exit 2
fi
"$OUT/test_local_offset.exe"
rc=$?
if [ "$rc" -eq 127 ] || [ "$rc" -gt 2 ]; then
  echo "FAIL: the binary did not run (exit $rc). This is the RIG, not the code."
  exit 2
...
if [ -z "$XCXX" ] || [ ! -x "$XCXX" ]; then
  ...
  echo "cross:    $XCXX"
  for tu in DrillMachine GameFormat Derive; do
...
for f in "$ROOT/src/game/DrillMachine.cpp" "$ROOT/src/game/DrillMachine.h" \
         "$ROOT/src/game/GameFormat.cpp" "$ROOT/src/game/GameFormat.h"          "$ROOT/src/game/Derive.cpp" "$ROOT/src/game/Derive.h"; do
  strip_comments "$f" \
