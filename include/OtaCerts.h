...
// Pinned root CAs for the OTA path (and only the OTA path): a spoofed feed puts
// bad data on a screen, but a spoofed firmware binary is persistent code
// execution, so OTA validates its TLS chain even though feeds stay setInsecure.
// Covers today's chains plus rotation headroom (roots expire 2035-2046):
//   github.com                    -> Sectigo E46 root (or its USERTrust ECC cross-sign)
//   *.githubusercontent.com (CDN) -> Let's Encrypt -> ISRG Root X1
// Sectigo R46 / USERTrust RSA+ECC / ISRG X2 / DigiCert G2+G3 are sibling and
// historical GitHub roots, kept so a CA rotation doesn't strand the fleet.
// Extracted from the Mozilla CA bundle (curl.se/ca/cacert.pem), 2026-07.
// mbedTLS parses concatenated PEMs from one buffer; certs are in this order:
...
constexpr const char OTA_ROOT_CAS[] = R"CERTS(
-----BEGIN CERTIFICATE-----
