...
<div class="wrap">
  ...
  <section class="blk">
    <h3>It’s showing the setup screen again</h3>
<p>
  Almost always this means your network wasn’t there when the scope powered on
  — a power cut, a router still restarting, or the scope being plugged in
  somewhere before the Wi-Fi came up.
</p>
<p>
  <strong>It has not forgotten your Wi-Fi.</strong> It keeps retrying your saved
  network in the background while that setup screen is up, roughly every five
  minutes, and joins on its own the moment the network is back.
</p>
<p>
  <strong>Give it up to five minutes before doing anything.</strong> You only
  need to run setup again if you are moving it to a genuinely different network
  — a new name or a new password.
</p>
...
<h2 id="problems">Troubleshooting</h2>
    ...
    <div class="fix">
      ...
      <p>You don't need a camera. Open
         ...
         displaying, right then.</p>
      <p><strong>On an iPhone or iPad:</strong> press and hold the picture and
         choose <em>Save to Photos</em>.</p>
      <p><strong>On Android:</strong> press and hold and choose
         <em>Download image</em>. Chrome may warn that this is an insecure
         download &mdash; that is because the device serves the page over plain
         HTTP on your own network, not because anything is wrong with the
         picture. Tap <em>Keep</em>.</p>
      <p><strong>On a computer:</strong> use the <em>Save image</em> button. Your
         browser may ask you to confirm for the same reason.</p>
  ...
  <section class="blk">
    ...
    <p>When a card shows an origin and destination, that comes from a database of
       ...
       flight plan.</p>
    <p><b>So a route is a best-effort lookup by flight number, not a live flight
       plan, and it can lag a schedule change.</b> Airlines reassign flight
       numbers between seasons; a database captured before a reassignment will
       confidently give you the old city pair. That has always been true of this
       field, whichever database was answering — the difference now is that ours
       is dated, refreshed daily, and we can tell you where it came from.</p>
