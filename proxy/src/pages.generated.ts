...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .verified{color:var(--live);font-size:12px;flex:none}
...
<footer>
  <div class="wrap">
    Opt in from your Blipscope’s config page. Counts only — no location or flight data leaves your device. ✓ marks a cloud-verified device.<br>
...
function renderRows(rows){
  ...
  board.innerHTML = rows.map((r,i) => {
    const bits = [];
    if (r.claimed?.types    != null) bits.push(\`\${nf(r.claimed.types)} types\`);
    if (r.claimed?.airlines != null) bits.push(\`\${nf(r.claimed.airlines)} airlines\`);
    if (r.claimed?.countries!= null) bits.push(\`\${nf(r.claimed.countries)} countries\`);
    if (r.claimed?.airports != null) bits.push(\`\${nf(r.claimed.airports)} airports\`);
    return \`<div class="row\${i===0?' top':''}">
      <div class="rank">\${i+1}</div>
      <div>
        <div class="who">
          <span class="name">\${esc(r.name || 'Unnamed scope')}</span>
          \${r.verified ? '<span class="verified" title="Cloud-verified device">✓</span>' : ''}
        </div>
